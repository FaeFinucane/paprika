...
uses the named baseline method to strengthen existing HIDDEN->HIDDEN contacts
from the active symbol's hidden feature group with bounded updates; normal
hidden-to-output teacher alignment remains active.
...
Each reward-STDP training trial is evaluated before its scalar reward is applied:
